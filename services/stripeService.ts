...
// Soporte dual para variables estándar de Vite y variables portadas de Next.js
const STRIPE_PUBLIC_KEY = getEnv('VITE_STRIPE_PUBLIC_KEY') || getEnv('NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY');

export const getStripe = () => {
    if (!STRIPE_PUBLIC_KEY) {
        console.error('Error: No se ha configurado la clave pública de Stripe (VITE_STRIPE_PUBLIC_KEY).');
    }
    ...
    if (STRIPE_PUBLIC_KEY.startsWith('pk_test')) {
        if (isProduction) {
            console.error('%c⚠️ ALERTA CRÍTICA: Se está utilizando una clave de PRUEBAS en PRODUCCIÓN. Los pagos reales no funcionarán.', 'color: white; background: red; font-size: 16px; font-weight: bold; padding: 8px;');
        } else {
...
export const createPaymentIntent = async (amountCents: number, userId: string, itemKey: string, metadata: Record<string, any> = {}) => {
    ...
    // El checkout real se gestiona a través de Edge Functions para máxima seguridad
    const { data, error } = await supabase.functions.invoke('payment-sheet', {
        body: { amount: Math.round(amountCents), description: `Pago ${itemKey}`, metadata: { ...metadata, userId, itemKey } }
    });
