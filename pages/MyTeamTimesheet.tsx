...
const MyTeamTimesheet: React.FC = () => {
  ...
  const TaskCard: React.FC<{ task: Task }> = ({ task }) => (
    <div className={`bg-gray-800 p-4 rounded-xl shadow-lg border-l-4 ${(task.status === 'done' || task.status === 'done') ? 'border-gray-500' : 'border-fuchsia-500'}`}>
      <div className="flex justify-between items-start">
        <h3 className={`font-semibold text-lg ${(task.status === 'done' || task.status === 'done') ? 'text-gray-500 line-through' : 'text-white'}`}>{task.description}</h3>
        <button 
          onClick={() => toggleTask(task.id)}
          className={`p-1 rounded-full transition-colors ${(task.status === 'done' || task.status === 'done') ? 'bg-gray-700 text-gray-400 hover:text-white' : 'bg-green-700 text-white hover:bg-green-600'}`}
          aria-label={(task.status === 'done' || task.status === 'done') ? 'Marcar como Pendiente' : 'Marcar como Completada'}
        >
          {(task.status === 'done' || task.status === 'done') ? <ListTodo className="w-5 h-5" /> : <CheckCircle className="w-5 h-5" />}
        </button>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2">
            ...
            <div className="space-y-4">
              {relevantTasks.filter(t => !(t.status === 'done' || t.status === 'done')).map(task => (
                <TaskCard key={task.id} task={task} />
              ))}
              {relevantTasks.filter(t => !(t.status === 'done' || t.status === 'done')).length === 0 && <p className="text-gray-500 text-center py-4">¡No tienes tareas pendientes!</p>}
