                                ...
                                {tasks.map(task => (
                                    <li key={task.id} className="flex items-center justify-between p-3 bg-gray-800/50 rounded-lg">
                                        <div className="flex items-center gap-3">
                                            <button onClick={() => toggleTask(task.id)} aria-label={(task.status === 'done' || task.status === 'done') ? `Marcar tarea '${task.description}' como incompleta` : `Marcar tarea '${task.description}' como completada`}>
                                                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${(task.status === 'done' || task.status === 'done') ? 'border-primary-500 bg-primary-500' : 'border-gray-500'}`}>
                                                    {(task.status === 'done' || task.status === 'done') && <svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>}
                                                </div>
                                            </button>
                                            <span className={` ${(task.status === 'done' || task.status === 'done') ? 'line-through text-gray-500' : 'text-white'}`}>{task.description}</span>
                                        </div>
