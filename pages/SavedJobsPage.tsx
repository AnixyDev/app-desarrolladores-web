...
const ProposalGeneratorModal = lazy(
  () => import('@/components/modals/ProposalGeneratorModal')
);
const UpgradePromptModal = lazy(
  () => import('@/components/modals/UpgradePromptModal')
);
      ...
      {savedJobs.length === 0 ? (
        ...
        <div className="space-y-4">
          {savedJobs.map(job => (
            <div
              key={job.id}
              className="bg-gray-900 p-4 rounded-xl border border-gray-700 flex flex-col sm:flex-row justify-between items-center gap-4"
            >
              <div className="flex-1">
                <Link
                  to={`/job-market/${job.id}`}
