...
const AIAssistantPage: React.FC = () => {
    ...
    const [messages, setMessages] = useState<Message[]>([
        { id: 1, text: 'Hola. Soy tu **Consultor Senior de Estrategia**. ¿Cuál es tu desafío hoy?', sender: 'ai' }
    ]);
    ...
    const handleSend = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const aiText = String(response.text || "No se pudo obtener una respuesta clara.");
            ...
            // FIX: Extraer solo el string del mensaje para evitar Error #31
            const errorMessage = error?.message || "Ocurrió un error inesperado al consultar la IA.";
            setMessages(prev => [...prev, { id: Date.now(), text: `⚠️ Error: ${String(errorMessage)}`, sender: 'ai' }]);
                    ...
                    {messages.map((message) => (
                        <div key={message.id} className={`flex items-start gap-4 ${message.sender === 'user' ? 'flex-row-reverse' : ''}`}>
                            ...
                            <div className={`max-w-[85%] p-5 rounded-2xl text-sm leading-relaxed ${
                                ...
                                <p style={{ whiteSpace: 'pre-wrap' }}>{String(message.text)}</p>
                            </div>
                        </div>
                    ))}
                    {isLoading && (
                ...
                <div className="p-4 border-t border-gray-800 bg-gray-900/80">
                    <form onSubmit={handleSend} className="flex gap-4">
                        <input
                            ...
                            disabled={isLoading}
                        />
                        <Button type="submit" disabled={isLoading || !input.trim()} className="shrink-0 w-14 h-14 rounded-2xl shadow-xl shadow-primary-600/20">
